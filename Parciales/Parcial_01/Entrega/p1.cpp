/*  Rebote simple - solucion:
    Se simula el rebote de un bola de ping pong cuando la raqueta (pared inferior) es fija.
...
void Colisionador::CalculeFuerzaPared(Cuerpo & Particula1, Cuerpo & Particula2){
  ...
  if (s > 0){
    ...
    F = K*pow(s,1.5) - Gamma*Particula1.m*Vy*pow(s,0.5);       //fuerza de la colision
    if(F > 0){
    vector3D Fr = F*n;
...
void InicieAnimacion(void){
  ...
  cout<<"set output 'rebote_simple.gif'"<<endl;
  cout<<"unset key"<<endl;
...
void InicieCuadro(void){
  cout<<"plot 0,0";   
  cout<<" , "<<Lx/7<<"*t,0";        //raqueta (pared inferior)
}
...
int main(void){
  ...
  double m0 = 1, R0 = 2;
  double t, tdibujo, tmax = 13, tcuadro = tmax/100, dt = 0.0001;
  int i;
