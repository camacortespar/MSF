/*
  Parcial 02/Punto 03: Difusion 2D - Flujo de Poiseuille
...
double LatticeBoltzmann::Jx(int ix, int iy, bool UseNew){
  //Propiedades del perfil parabólico en x
  double delta_p = 0.001, eta = 10.0, Jx_aux;
  Jx_aux = 0.5*rho(ix,iy,false)*(delta_p/eta)*iy*(Ly-1-iy);
...
void LatticeBoltzmann::Start(double rho0, double Ux0, double Uy0){
  int ix, iy, i, n0;
  double sigma = Ly/9, mu = Lx/8, coef = 1/(sigma*sqrt(2*M_PI));  //informacion gaussiana
  double rho_gauss;
...
int main(void){
  ...
  for(t=0; t<tmax; t++){
    ...
    Air.Advection();
    if(t==500){
      Air.Print("p3_500.dat", Ufan0);
    }
    if(t==1000){
      Air.Print("p3_1000.dat", Ufan0);
    }
  }
  //Imprima
  Air.Print("p3_2000.dat", Ufan0);
